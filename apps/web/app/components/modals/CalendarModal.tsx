...
export const CalendarModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  const getDayClass = (dateStr: string) => {
    if (blockedDates.includes(dateStr)) return "bg-slate-100 text-slate-400 cursor-not-allowed";
    if (rangeAnchor === dateStr) return "bg-indigo-800 text-white ring-2 ring-indigo-300 ring-offset-2 z-10 scale-105";
    if (pendingDates.includes(dateStr)) return "bg-indigo-600 text-white shadow-sm";
      ...
      >
        ...
        <div className="p-5 bg-slate-800 text-white flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-slate-400 text-sm">Manage blackout dates</p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row h-full overflow-hidden">
              ...
              <div className="grid grid-cols-7 gap-2">
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  ...
                  return (
                    ...
                    >
                      <span>{day}</span>
                      {hasEvent && !isPending && !isBlocked && <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1"></div>}
          ...
          <div className="w-full lg:w-80 bg-slate-50/50 flex flex-col min-h-[200px] lg:min-h-0">
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-2">
                {groupedBlockedDates.length === 0 ? (
                    ...
                    groupedBlockedDates.map((group, idx) => {
                        ...
                        return (
                            <div key={idx} className="group flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 hover:border-indigo-300 hover:shadow-sm transition-all">
                                <div className="flex items-center gap-3">
                                    <div className={`w-1 h-8 rounded-full ${isRange ? 'bg-indigo-500' : 'bg-slate-400'}`}></div>
                                    <div>
                                        <p className="text-sm font-bold text-slate-700">{label}</p>
                                        {isRange && <p className="text-[10px] text-slate-400 font-medium">{group.length} days</p>}
                                    </div>
                                </div>
                                <button 
